...
echo "then, for (hdfs and hbase), manually copy master's pubkey in(/home/hadoop/.ssh/authorized_keys) to others' workers authorized_keys"
echo "then, Run configure-... files on hdfs master (syncs conf auto), on all znodes, on hbase master (syncs conf auto)"